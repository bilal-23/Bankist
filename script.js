...
function calcPrintBalance(acc) {
    const balance = acc.movements.reduce((total, value) => { return total + value });
...
function calcDisplaySummary(acc) {
    const income = acc.movements.filter(x => x > 0).reduce((total, cur) => { return total + cur }, 0);
    labelSumIn.textContent = `${income}€`;

    const outgoing = acc.movements.filter(x => x < 0).reduce((total, cur) => { return total + cur }, 0);
